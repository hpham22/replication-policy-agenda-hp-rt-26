...
- **t-fit mu:** 0.0000 (SE = NA)
- **t-fit sigma:** 0.0000 (SE = NA)
- **t-fit nu:** 1.0100 (SE = NA)
...
- Shapiro-Wilk: W = 0.299039, p = 5.57e-47
- t-fit: mu = 0.0000, sigma = 0.0000, nu = 1.0100 (converged)

### Core (n = 132)
- Kurtosis: 42.4905 | L-kurtosis: 0.4199
- Shapiro-Wilk: W = 0.503274, p = 3.06e-19
- t-fit: mu = 0.0000, sigma = 0.0000, nu = 1.0164 (converged)
...
- **T-distribution fitting:** The MLE fits produce degenerate values (sigma near 0, nu at the
  lower bound of 1.01) because 83% of pooled area-level percentage changes are exactly 0
  (from zero-to-zero transitions) and 10% are exactly -1 (positive-to-zero). The continuous
  t-distribution cannot adequately model data with such extreme point masses. The kurtosis
  (170.26) and L-kurtosis (0.6463 vs. normal benchmark 0.1226) provide stronger evidence
  for punctuated equilibrium than the t-fit parameters in this case. The core subset shows
  kurtosis = 42.49 (lower than aggregate but still extreme), while peripheral shows 11.22.
  Both are far above the normal benchmark of 3.
