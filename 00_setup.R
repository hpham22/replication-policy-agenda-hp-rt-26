...
# --- Path configuration ---
if (exists("repo_root") && !is.null(repo_root)) {
  # Already set by caller (e.g., run_all.R)
} else {
  candidate <- getwd()
...
data_file    <- file.path(repo_root, "legal_instrument_data.xlsx")
results_dir  <- file.path(repo_root, "results")

# Backward-compatible aliases (all outputs go to results/)
...
save_figure <- function(plot, name, width = 6.5, height = 4.5) {
  pdf_path <- file.path(results_dir, paste0(name, ".pdf"))
...
save_table <- function(tbl, name, caption = NULL, footnote = NULL,
                       ...
                       pack_rows = NULL) {
  csv_path <- file.path(results_dir, paste0(name, ".csv"))
